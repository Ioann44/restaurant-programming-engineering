...
export class RestaurantController {
	...
	async getAll(@Body() input: DateDto): Promise<RestaurantDto[]> {
		const usedTables = !("date" in input) ? new Set([] as number[]) : await this.reservationsService.getTablesOnDate(input.date);
		...
		for (const item of restaurants) {
			...
			item.menu = item.menu.map(item => this.dishService.entityToDto(item)) as any;
	...
	@Get("one/:id")
	async getOne(@Param("id") id: number, @Body() input: DateDto): Promise<RestaurantDto> {
		const usedTables = !("date" in input) ? new Set([]) : await this.reservationsService.getTablesOnDate(input.date);
		...
		const menu = restaurant.menu.map(item => this.dishService.entityToDto(item));
